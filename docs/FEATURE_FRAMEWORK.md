...
- `上传透明头像` 属于实用功能，设置页放在 `美化` 分组，默认关闭。开启后 Hook Android `Bitmap.compress(Bitmap.CompressFormat,int,OutputStream)`，仅当调用栈属于 `com.tencent.mm.modelavatar`、`PhotoCropActivity` 或微信媒体裁剪类时把输出格式切换为 PNG，以保留个人头像的 Alpha 透明通道；普通聊天图片、朋友圈图片和 Hchat 自身的图片压缩保持原格式。媒体裁剪类使用稳定字符串 `Rect width or height contains zero. contentRect: ` 经 DexKit 定位，要求候选唯一，并按微信包、版本、APK、热更新和 ClassLoader 组成的运行时 key 缓存；定位失败时两个稳定头像调用栈仍可工作。本功能基于随附源码及其字符串锚点迁移，尚未完成微信 `8.0.49` 至 `8.0.77` 的逐版本真机验证；部分微信流程上传时可能需要先缩放一次再确认。
- 模块设置页“关于”区域提供“推送更新”：进入关于页后在后台请求 `Flandre6/Hchat-alt-entry` 的 GitHub Latest Release，显示当前版本、最新版本、更新说明，并可打开 APK 资产或 Release 页面下载；主进程启动后也会按 12 小时节流执行同一检查，发现高于当前 `versionName` 的 Release 时，通过宿主系统通知提示一次，并按 Release tag 去重。网络失败只显示重试状态，不阻塞微信启动和消息处理线程；Android 13 及以上在通知权限未授予时不标记为已通知，待权限可用后可再次提醒。
...
`securemessage` 提供两个相互独立的设置入口：`安全消息` 在自己发送的文字、图片、视频（含 type 62）、
表情和 AppMsg 入库前加入 `sec_msg_node` 标记；链接、普通卡片和引用回复均属于 AppMsg。除本地入库兜底外，
功能还会从已确认的消息入库参数类派生单字符串 setter 候选，仅在参数确实是完整 `msgsource` 时合并标记；setter 阶段只排除已明确为接收方向的 `isSend=0`，读取不到或尚未初始化方向的媒体消息继续按发送链处理，使图片、视频、表情和 AppMsg 在提交请求前也能获得标记。`反安全消息` 会关闭微信对该标记的识别检查，并在已确认的本地消息入库
...
当反安全消息关闭时，安全消息菜单会独立于消息入库和媒体发送定位，在同一菜单创建入口的最高优先级收尾阶段按当前 `View.tag` 重新识别消息；
微信 8.0.77 的媒体气泡 `View.tag` 是 `ChattingItem` 包装对象，模块会沿有限对象链取得其 `MsgInfo`，并兼容空的 `field_msgSource` 回退到混淆字段/访问器。
仅保留标题为删除/`delete`/`remove` 的菜单项，覆盖文字、链接、卡片、图片、视频和表情以及其它模块追加的菜单项；
无法识别删除项时保守保留原菜单并记录错误，避免误删普通消息。反安全消息开启时跳过该限制，以保持完整菜单。
`安全消息` 默认开启，`反安全消息` 默认关闭；运行时分别通过 `Hchat_secure_message` 和
`Hchat_anti_secure_message` 配置。反安全消息只有用户明确打开后才恢复完整菜单；
...
安全消息的表情发送链路额外使用精确版本配置：8.0.76/3140 Hook
`lq1.d1.a(e9): String` 与 `o22.y.doScene(network.s, modelbase.u0): int`；8.0.77/3160 Hook
`ft1.d1.a(e9): String` 与 `k52.y.doScene(network.s, modelbase.u0): int`。第一条链路修改表情上传阶段取得的
MsgSource，第二条链路在 `sendemoji` 请求分发前再次修改首项 `p` 字段，覆盖上传阶段重建请求的情况。
这两条链路只受 `安全消息` 总开关控制，不读取联系人或群聊白名单，对所有发送目标生效；未知版本不安装精确
表情 Hook，继续使用通用入库和 setter 兜底。当前仅有 8.0.76、8.0.77 的静态逆向证据，尚未完成真机验证。
