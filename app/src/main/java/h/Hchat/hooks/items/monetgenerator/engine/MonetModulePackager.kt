...
object MonetModulePackager {
    ...
    fun pack(
        ...
        ZipOutputStream(output.outputStream().buffered()).use { zip ->
            ...
            add(
                "module.prop",
                "id=hchat-monet-engine\nname=微信莫奈引擎 (Hchat)\n" +
                    "version=$versionName ($versionCode)\nversionCode=$versionCode\n" +
                    "author=Hchat contributors, based on Flandre6/WeKit\n" +
                    "description=为微信 $versionName 启用动态壁纸取色, 由 Hchat 在运行时生成\n",
            )
...
select_tab_overlay() {
  ...
  case "$style" in
    solid)
      name=HchatMonetWeChatSolidTab
      package=hchat.monet.solidtab.com.tencent.mm
      ;;
    blur)
      name=HchatMonetWeChatBlurTab
      package=hchat.monet.blurtab.com.tencent.mm
      ;;
    *) return 1 ;;
  esac
  ...
  if [ "$(getprop ro.build.version.sdk)" -ge 34 ]; then
    rm -rf "$MODDIR/system/priv-app/HchatMonetWeChatSolidTab" "$MODDIR/system/priv-app/HchatMonetWeChatBlurTab"
    ...
    mkdir -p "$target" || return 1
    rm -f "$target/HchatMonetWeChatSolidTab.apk" "$target/HchatMonetWeChatBlurTab.apk"
    cp -f "$source" "$target/$name.apk" || return 1
  fi
...
MODDIR=${0%/*}
LOCK=/dev/.hchat-monet-overlay-restore
...
"""
    ...
    private const val SOLID_TAB_PACKAGE = "hchat.monet.solidtab.com.tencent.mm"
    private const val BLUR_TAB_PACKAGE = "hchat.monet.blurtab.com.tencent.mm"
}
